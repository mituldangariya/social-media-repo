...
//===== Search Filter =====//
	...
	/*document.addEventListener('DOMContentLoaded', function () {
		...
		document.querySelector('.widget.friend-list').prepend(searchInput);

		const searchDir = document.getElementById('searchDir');
		const userList = document.getElementById('userList');
		const itemsToSearch = Array.from(userList.getElementsByTagName('li')).map(li => li.textContent.trim());
		...
		function displayResults(results) {
			searchDir.innerHTML = ''; // Clear previous results

			results.forEach(result => {
				const resultElement = document.createElement('div');
				resultElement.textContent = result;
				searchDir.appendChild(resultElement);
			});
		}
	});*/
